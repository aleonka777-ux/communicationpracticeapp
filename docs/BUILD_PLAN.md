...
- [x] Phase 4A — speech-delivery EVIDENCE collection (speaking rate, intra-utterance pauses, filler/disfluency candidates, relative vocal intensity): `src/lib/realtime/sessionTimeline.ts` (WPM + filler candidates, from transcript text), `speechDeliveryTracker.ts` + `micEnergyMonitor.ts` (live mic-energy-based pause/intensity detection), `fillerCandidates.ts` (candidate detection, never a judgment), `mergeSpeechDeliveryEvidence.ts`. New additive migration `0014_speech_delivery_evidence.sql` (`realtime_pause_events`, `realtime_disfluency_candidates` tables + new columns on the two existing tables). Audited `whisper-1` (the current Realtime transcription model) and found it unreliable for vocal-disfluency (um/uh) preservation — that evidence category is explicitly best-effort/undercount-only; no model switch made without empirical verification. `VOCAL_EVIDENCE_AVAILABLE` unchanged (`false`) — evidence only, no interpretation/scoring/coaching use. See `docs/DECISIONS.md` "Phase 4A: speech-delivery evidence."
- [ ] Not yet: Training Mode hints wired into the Realtime screen
- [ ] Not yet: Phase 4B — interpreting/classifying the Phase 4A evidence (which pause is strategic vs. hesitant, which candidate is a genuine filler, how speaking rate/intensity relate to context) against the Communication Manual methodology
- [ ] Not yet: raw audio persistence
- [ ] Not yet: using the new timing/interruption/speech-delivery metrics in Evaluation Engine scoring or any production-facing UI
- [ ] Not yet: verified against a live OpenAI account in production (see /docs/STATUS.md) — Phase 4A's live mic-energy analysis (Web Audio AnalyserNode) is additionally unverified against a real browser/microphone in this build environment
