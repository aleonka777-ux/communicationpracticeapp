...
- `realtime_session_metrics` — one row per session (1:1, upserted at finalization), the derived aggregate timing/interruption metrics (speaking time/percentage, overlap, confirmed-interruption count, response latency), plus (as of Phase 4A) speech-delivery aggregates (speaking rate, filler-candidate counts/rates, pause counts/durations). A measurement layer only — not yet read by the Evaluation Engine or shown in production UI.
- `realtime_pause_events` — one row per detected intra-utterance pause (Phase 4A). Timestamps/durations/position only, never raw audio. See §9.3 and `docs/DECISIONS.md` "Phase 4A: speech-delivery evidence."
- `realtime_disfluency_candidates` — one row per filler/disfluency CANDIDATE occurrence (Phase 4A) — never a judgment; `classification` is DB-constrained to `'unclassified'` today.
...
- `practice_sessions`, `conversation_messages`, `evaluations`, `realtime_turn_events`, `realtime_session_metrics`, `realtime_pause_events`, `realtime_disfluency_candidates`: owner-only (`user_id = auth.uid()`, the latter six via a subquery on the parent session's `user_id`). No cross-user visibility, including for coaches — coaches manage methodology, not other users' transcripts, in this MVP.
...
- **Phase 4A — speech-delivery evidence**, layered alongside the above: `sessionTimeline.ts` also derives speaking rate (WPM) and filler/disfluency candidates (`src/lib/realtime/fillerCandidates.ts`) from the same confirmed-turn transcripts it already has. A second, independent module (`speechDeliveryTracker.ts`, fed by the browser-only `micEnergyMonitor.ts` reading the LOCAL mic stream via a Web Audio `AnalyserNode`) detects intra-utterance pauses and relative (never dB SPL) vocal intensity, since server VAD only delimits whole turns. `mergeSpeechDeliveryEvidence.ts` combines both into the payload posted alongside the timing metrics, filtering pause/intensity evidence to confirmed turns only. Same privacy/failure-isolation guarantees as the timing layer: no raw audio, analytics failure never blocks the conversation. See `docs/DECISIONS.md` "Phase 4A: speech-delivery evidence" for the full design, including why vocal-disfluency evidence is explicitly best-effort/undercount-only given the current `whisper-1` transcription pipeline.
